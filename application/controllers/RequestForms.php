 ...
 * @author MIS
 */
class RequestForms extends MY_Controller {
    ...
    public function index() {
        if ($this->has_logging_in()) {
            $data["page_title"] = "Request Forms";
            $data['page'] = 'pages/menu/my_account/request_forms/request_forms';
    ...
    public function FetchWorkschedule() {
        ...
        if (count($result) > 0) {
            ...
            if ($form == 4) {
                $data['time_in'] = $this->ConvertTo12Format($result[0]->timein);
                $data['time_out'] = $this->ConvertTo12Format($result[0]->timeout);
            } else {
